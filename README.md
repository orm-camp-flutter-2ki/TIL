...
스트림은 일련의 이벤트들을 비동기적으로 전달하는 개념

이벤트들은 시간에 따라 발생하며, 데이터의 흐름을 표시

## Dart의 스트림

Dart에서 스트림은 'dart:async' 패키지를 통해 지원

'Stream' 클래스는 데이터의 흐름을 나타내며, 'StreamController'를 사용하여 데이터를 생성하고 전송

이벤트를 수신하는 쪽은 'listen()' 메서드를 사용하여 스트림에 대한 구독을 시작

## Flutter에서의 스트림

Flutter는 Dart의 스트림을 활용하여 UI 업데이트와 관련된 비동기 작업을 처리

애니메이션, 사용자 입력 및 네트워크 요청과 같은 작업은 스트림을 통해 처리

Flutter에서는 'StreamBuilder' 위젯을 사용하여 스트림에서 발생하는 데이터를 UI로 표시

## 스트림의 장점

비동기적 작업을 보다 쉽게 처리

데이터의 흐름을 명확하게 표현할 수 있어 코드의 가독성을 향상

이벤트 기반 프로그래밍을 지원하여 복잡한 상황에서도 코드를 간결하게 유지
-------------------
알고리즘 학습 필요
